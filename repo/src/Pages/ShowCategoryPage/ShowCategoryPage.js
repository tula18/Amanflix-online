    ...
    const fetchGenres = async () => {
        try {
            ...
            for (const genreId in data) {
                ...
                const url = `${API_URL}/cdn/${isLoggedIn ? 'auth-search' : 'search'}?media_type=tv&with_images=true&genre=${encodedGenre}&page=page_num&per_page=36&include_watch_history=true`
                newCategories[genre] = {
                    type: 'genre',
                    title: `${String(capitalizedGenre)} Shows`,
                    url: url
                }
            }
        ...
        <div className='ShowCategoryPage'>
            ...
            {movies.length === 0 ? (
                <p className='category-status-message'>
                    <b>No shows match these filters.</b>
                </p>
            ) : (
                <InfiniteScroll
