...
const getCategoryEyebrow = (category) => {
    ...
    if (category.type === 'genre') return 'Movie genre';
...
const getCategoryDescription = (category) => {
    if (!category) return 'Browse movies from the Amanflix catalog.';
    ...
    if (category.title.toLowerCase().includes('random')) {
        return 'A shuffled selection of movies for when you want something unexpected.';
    }
    if (category.title.toLowerCase().includes('uploaded')) {
        return 'Browse the uploaded movie library in one place.';
    }
    return 'Browse movies from the Amanflix catalog.';
};
